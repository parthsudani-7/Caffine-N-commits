  ...
  return (
    <PageWrapper direction={direction}>
      <div className="min-h-screen bg-[#03152A] text-white p-6">
        ...
        <div className="mb-10">
          <div className="text-sm mb-2 text-gray-400">Zone Risk Score</div>
          <div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${risk}%` }} transition={{ duration: 1 }} className="h-full bg-[#00B8A0]" />
          </div>
          <div className="mt-2 text-[#00B8A0] font-bold">{risk}% RISK</div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((tier, index) => (
            <motion.div
              ...
              className={`p-6 rounded-xl cursor-pointer border ${selected === index ? "border-[#00B8A0] bg-[#06263d]" : "border-white/10 bg-white/5"}`}
            >
              <h2 className="text-xl font-semibold mb-2">{tier.name}</h2>
              <p className="text-sm text-gray-400 mb-4">{tier.coverage}</p>
              <div className="text-3xl font-bold text-[#00B8A0]">₹{tier.basePrice}</div>
              <div className="text-xs text-gray-400 mt-1">per week</div>
            </motion.div>
          ))}
        </div>
